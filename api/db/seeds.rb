...
ActiveRecord::Base.transaction do
  ...
  demo_companies = SeedData.build_companies_for(
    user: demo_user,
    names: demo_company_names,
    locations: locations,
    note_pool: company_notes
  )
  demo_tags = SeedData.build_tags_for(user: demo_user, tag_data: demo_tag_data)
  ...
  demo_status_targets.each do |status, count|
    count.times do |idx|
      app = SeedData.create_application(
        user: demo_user,
      ...
      SeedData.assign_tags(application: app, tags: demo_tags)
      SeedData.create_notes_and_events_for(application: app, status: status, note_templates: note_templates)
    end
  end
  ...
  scoped_companies = SeedData.build_companies_for(
    user: scoped_user,
    names: scoped_company_names,
    locations: locations.rotate(3),
    note_pool: company_notes
  )
  scoped_tags = SeedData.build_tags_for(user: scoped_user, tag_data: scoped_tag_data)
  ...
  scoped_status_targets.each do |status, count|
    count.times do |idx|
      ...
      SeedData.assign_tags(application: app, tags: scoped_tags)
      SeedData.create_notes_and_events_for(application: app, status: status, note_templates: note_templates)
    end
  end
